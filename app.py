...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .fraud-card {
        ...
        border: 2px solid #ff3333; border-radius: 12px;
...
def load_models():
    for path, file_id in FILES.items():
        if not os.path.exists(path):
            with st.spinner(f"Téléchargement {path}..."):
...
with st.sidebar:
    st.markdown("### 📊 Modèle")
    st.markdown("**Algorithme :** Extra Trees")
    st.markdown("**PR-AUC :** 0.8808")
    st.markdown("**Recall :** 0.8469")
    st.markdown("**Precision :** 0.9326")
